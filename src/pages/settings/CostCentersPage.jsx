  ...
  return (
    <form onSubmit={submit} className="premium-card p-4 grid sm:grid-cols-2 gap-3">
      ...
      <input value={form.description} onChange={set('description')} placeholder="Description (optional)"
        className="sm:col-span-2 px-3 py-2 rounded-lg border border-glass bg-glass-panel/40 text-small text-text-primary focus:outline-none focus:border-accent/40" />
    ...
    <div className="animate-fade-in pb-10 space-y-5 max-w-4xl">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-xl bg-accent/15"><Building2 className="h-5 w-5 text-accent" /></div>
          <div>
            ...
            <div key={cc._id} className="premium-card p-3.5 flex items-center gap-3">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-small font-semibold text-text-primary">{cc.code}</span>
                  <span className="text-small text-text-secondary">{cc.name}</span>
                  <span className="text-label uppercase tracking-wider text-text-muted">{TYPE_LABEL[cc.type] || cc.type}</span>
                  {!cc.isActive && <span className="text-label text-highlight">inactive</span>}
                </div>
                {cc.parentId && <p className="text-label text-text-muted mt-0.5">under {nameOf(cc.parentId) || '—'}</p>}
              </div>
              <button onClick={() => setEditing(cc)} aria-label="Edit" className="p-1.5 rounded-lg text-text-muted hover:text-accent hover:bg-glass-hover"><Pencil className="h-4 w-4" /></button>
