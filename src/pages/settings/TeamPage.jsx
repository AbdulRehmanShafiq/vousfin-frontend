    ...
    <div className="flex flex-wrap gap-2">
      {ROLES.map(r => (
        <button key={r.v} type="button" onClick={() => toggle(r.v)} title={r.desc}
          className={`px-2.5 py-1 rounded-lg text-[12px] border transition ${value.includes(r.v)
            ? 'border-cyan/50 bg-cyan/15 text-text-primary'
  ...
  return (
    <div className="animate-fade-in pb-10 space-y-5 max-w-4xl">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-xl bg-cyan/15"><UsersRound className="h-5 w-5 text-cyan" /></div>
        <div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); if (email.trim() && inviteRoles.length) invite.mutate() }}
        ...
        <div className="flex items-center gap-2 text-[12.5px] font-semibold text-text-primary"><UserPlus className="h-4 w-4 text-cyan" /> Invite someone</div>
      ...
      {isLoading ? (
        ...
        <div className="premium-card p-6 text-center text-[13px] text-text-secondary">No team members yet — invite someone above.</div>
      ) : (
              ...
              <div className="flex items-center gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-[13px] font-semibold text-text-primary">{memberName(m)}</span>
                    {m.status !== 'active' && <span className="text-[10.5px] uppercase tracking-wider text-amber">{m.status}</span>}
                  </div>
                  {m.userId?.email && <p className="text-[11.5px] text-text-muted mt-0.5">{m.userId.email}</p>}
                </div>
