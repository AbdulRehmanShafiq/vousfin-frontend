...
export default function BudgetEditorPage() {
  ...
  const renderGroup = (type, heading) => {
    ...
    return (
      <>
        <tr><td colSpan={14} className="pt-3 pb-1 text-label uppercase tracking-wider text-text-muted">{heading}</td></tr>
        {group.map((a) => (
          <tr key={a._id} className="border-b border-glass/50">
            <td className="py-1.5 pr-2 text-small text-text-primary whitespace-nowrap sticky left-0 bg-glass-panel/40">{a.accountName}</td>
            <td className="py-1.5 px-1">
              <input type="number" value={annualOf(monthly[a._id]) || ''} placeholder="year"
                onChange={(e) => applyAnnual(a._id, e.target.value)}
                className="w-20 px-1.5 py-1 rounded border border-glass bg-glass-panel/40 text-xs text-right text-text-primary focus:outline-none focus:border-accent/40" />
            </td>
            {Array.from({ length: 12 }).map((_, m) => (
              <td key={m} className="py-1.5 px-0.5">
                <input type="number" value={cell(a._id, m) || ''}
                  onChange={(e) => setCell(a._id, m, e.target.value)}
                  className="w-16 px-1 py-1 rounded border border-glass bg-glass-panel/30 text-label text-right text-text-secondary focus:outline-none focus:border-accent/40" />
              </td>
            ))}
          </tr>
        ))}
  ...
  return (
    <div className="animate-fade-in pb-10 space-y-5">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-xl bg-accent/15"><Target className="h-5 w-5 text-accent" /></div>
        <div>
      ...
      <div className="premium-card p-4 flex items-end gap-3 flex-wrap">
        <label className="flex flex-col gap-1">
          <span className="text-label text-text-muted">Year</span>
      ...
      {fiscalYears.length === 0 && (
        <div className="premium-card p-6 flex flex-col items-center text-center gap-3">
          <div className="p-2.5 rounded-xl bg-highlight/15"><CalendarDays className="h-5 w-5 text-highlight" /></div>
          <div>
            <p className="text-sm font-semibold text-text-primary">Set up a financial year first</p>
      ...
      {canEdit && rows.length > 0 && (
        <div className="premium-card p-4 space-y-3">
          ...
          <p className="text-label text-text-muted">Tip: type a yearly figure in the “Year” box to spread it evenly across the months, then tweak any month.</p>
          ...
          <div className="flex items-center gap-2">
            ...
            {budgetId && (
              <button onClick={() => submit.mutate()} disabled={submit.isPending}
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-small font-semibold text-positive hover:bg-glass-hover disabled:opacity-50">
