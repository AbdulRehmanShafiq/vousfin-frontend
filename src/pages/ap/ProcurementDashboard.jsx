    ...
    <div className="space-y-1.5">
      {data.slice(-6).map((row) => (
        <div key={row.month} className="flex items-center gap-2">
          <span className="text-[12px] text-text-muted w-14 shrink-0">{row.month}</span>
          ...
          <span className={`text-[12px] font-medium w-10 text-right ${(row.onTimeRate ?? 0) >= 80 ? 'text-positive' : 'text-amber'}`}>
            {pct(row.onTimeRate)}
          </span>
        </div>
      ))}
        ...
        <thead>
          <tr className="border-b border-glass">
            {['Bill', 'Vendor', 'Due', 'Balance', 'State'].map(h => (
              <th key={h} className="text-left py-1.5 px-1 text-text-muted font-medium text-[12px] uppercase">{h}</th>
            ))}
          </tr>
        </thead>
        ...
        return (
          <div key={e._id ?? i} className="flex items-start gap-2">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[12.5px] text-text-secondary leading-tight">
              ...
              {[
                ...
                <div key={label} className="flex justify-between items-baseline py-1 border-b border-glass last:border-0">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[12px] text-text-muted">{data?.count ?? 0} bills</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-12 gap-3">
        <div className="col-span-8 premium-card p-4 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <Link to="/purchases/bills" className="text-[12.5px] text-cyan hover:underline">
              View all →
            </Link>
          </div>
