...
const ObligationsBar = memo(function ObligationsBar({ data }) {
  ...
  const colors = ['bg-accent', 'bg-accent', 'bg-highlight', 'bg-highlight', 'bg-negative', 'bg-negative']
  return (
    ...
    <div className="space-y-1.5">
      {data.slice(-6).map((row) => (
        <div key={row.month} className="flex items-center gap-2">
          <span className="text-xs text-text-muted w-14 shrink-0">{row.month}</span>
          <div className="flex-1 h-3 bg-glass rounded-full overflow-hidden flex">
            ...
            <div className="h-full bg-accent"     style={{ width: `${Math.round((row.on_time/ Math.max(row.total, 1)) * 100)}%` }} title={`On-time: ${row.on_time}`} />
            <div className="h-full bg-negative"     style={{ width: `${Math.round((row.late   / Math.max(row.total, 1)) * 100)}%` }} title={`Late: ${row.late}`} />
          </div>
          <span className={`text-xs font-medium w-10 text-right ${(row.onTimeRate ?? 0) >= 80 ? 'text-positive' : 'text-highlight'}`}>
            {pct(row.onTimeRate)}
          </span>
        </div>
      ))}
      <div className="flex items-center gap-3 pt-1">
        {[['bg-positive','Early'],['bg-accent','On-time'],['bg-negative','Late']].map(([c,l]) => (
        ...
        <thead>
          <tr className="border-b border-glass">
            {['Bill', 'Vendor', 'Due', 'Balance', 'State'].map(h => (
              <th key={h} className="text-left py-1.5 px-1 text-text-muted font-medium text-xs uppercase">{h}</th>
            ))}
          </tr>
        </thead>
          ...
          {bills.map(b => (
            <tr key={b._id} className="hover:bg-glass/20 transition-colors">
              <td className="py-1.5 px-1">
                <Link to={`/purchases/bills/${b._id}/edit`} className="text-accent hover:underline font-mono">
                  {b.billNumber}
                </Link>
              </td>
              <td className="py-1.5 px-1 text-text-muted max-w-[120px] truncate">{b.vendorSnapshot?.vendorName || '—'}</td>
              <td className="py-1.5 px-1 text-highlight">{fmtDate(b.dueDate)}</td>
          ...
          <div key={e._id ?? i} className="flex items-start gap-2">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-small text-text-secondary leading-tight">
                ...
                {e.entityRef && <span className="text-accent ml-1 font-mono">{e.entityRef}</span>}
              </p>
...
const EfficiencyTile = memo(function EfficiencyTile({ data }) {
  const rows = [
    { label: 'PO-backed bills', value: pct(data?.poBackedRate),     color: data?.poBackedRate >= 70 ? 'text-positive' : 'text-highlight' },
    { label: '3-way match pass', value: pct(data?.matchPassRate),   color: data?.matchPassRate >= 80 ? 'text-positive' : 'text-highlight' },
...
function AlertsStrip({ overdue, matchIssues, criticalVendors }) {
  const alerts = [
    ...
    (matchIssues ?? 0)            > 0 && { msg: `${matchIssues} 3-way match issues`, href: '/procurement/purchase-orders', color: 'bg-highlight/10 text-highlight border-highlight/20' },
...
const QUICK_ACTIONS = [
  { label: 'New Bill',           to: '/purchases/bills/new',                    icon: FileText,      color: 'text-accent'       },
  { label: 'New PO',             to: '/procurement/purchase-orders/new',        icon: ShoppingCart,  color: 'text-accent'    },
  { label: 'AP Workflow',        to: '/purchases/ap-workflow',                  icon: BarChart2,     color: 'text-highlight'  },
          ...
          <div className="flex items-center gap-1">
            {QUICK_ACTIONS.map(({ label, to, icon: Icon, color }) => (
              <Link
                key={label}
                to={to}
                className="flex items-center gap-1 px-2.5 py-1.5 bg-glass rounded text-xs text-text-secondary hover:text-accent transition-colors"
              >
          ...
          {forecastLoading ? <SkeletonLoader count={3} /> : (
            <div className="space-y-2">
              {[
                ...
                { label: 'Next 30d', data: forecast?.requirements?.next30,   color: 'text-highlight'   },
                { label: 'Next 60d', data: forecast?.requirements?.next60,   color: 'text-accent'     },
                ...
                <div key={label} className="flex justify-between items-baseline py-1 border-b border-glass last:border-0">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-text-muted">{data?.count ?? 0} bills</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-12 gap-3">
        <div className="col-span-8 premium-card p-4 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <Link to="/purchases/bills" className="text-small text-accent hover:underline">
              View all →
            </Link>
          </div>
