...
export default function Login() {
    ...
    return (
      <div className="w-full">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-semibold text-text-primary tracking-tight">{t('auth.login.mfa.title', 'Two-factor check')}</h1>
          <p className="mt-2 text-text-secondary text-sm">{t('auth.login.mfa.subtitle', 'Open your authenticator app and enter the 6-digit code.')}</p>
        </div>
        <div className="space-y-5">
          <Input
            label={t('auth.login.mfa.codeLabel', 'Authenticator code')}
            type="text"
            inputMode="numeric"
            maxLength={6}
            placeholder="123456"
          ...
          <Button fullWidth onClick={handleMfaVerify} loading={mfaLoading}>
            {t('auth.login.mfa.verify', 'Verify')}
          </Button>
          <button
            type="button"
          ...
          >
            {t('auth.login.mfa.backToLogin', 'Back to login')}
          </button>
        </div>
      </div>
    )
  ...
  return (
    <div className="w-full">
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-semibold text-text-primary tracking-tight">{t('auth.login.title', 'Welcome back')}</h1>
        <p className="mt-2 text-text-secondary">{t('auth.login.subtitle', 'Log in to manage your finances')}</p>
      </div>
        ...
        <div className="mb-6 rounded-lg border border-cyan/30 bg-cyan/5 p-4 text-sm text-text-secondary">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <Button type="button" variant="outline" onClick={handleContinue}>
              {t('action.continueTo', 'Continue to app')}
            </Button>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <Input
          label={t('label.email', 'Email Address')}
          type="email"
          icon={Mail}
