/**
 * MobileBills — phone-native Bills list (Mobile-First Redesign, pass 2).
 * Presentational only: BillsListPage owns data + mutations.
 */
...
export default function MobileBills({
  ...
  onRefresh, onNew, onOpen, onArchive,
}) {
  return (
    <MobilePage
      title="Bills"
      subtitle="What your suppliers charge you"
      cta={
        <button
          type="button"
          onClick={onNew}
        ...
        <div className="space-y-3 pb-4">
          ...
          {isLoading && !rows.length ? (
            ...
            <p className="rounded-xl bg-glass-panel p-6 text-center text-sm text-text-muted">
              {query ? 'No bills match your search.' : 'No bills yet. Tap "New bill" to record your first one.'}
            </p>
          ) : (
                ...
                return (
                  <SwipeRow key={r._id} actions={actions}>
                    <ListCard
                      ...
                      title={r.vendorSnapshot?.vendorName || 'Vendor'}
                      subtitle={`${r.billNumber} · ${formatDate(r.issueDate)}`}
                    ...
                    <div className="px-3 pb-2 -mt-1">
                      <InvoiceStatusBadge state={r.state} kind="bill" size="sm" />
                    </div>
                  </SwipeRow>
                )
              })}
