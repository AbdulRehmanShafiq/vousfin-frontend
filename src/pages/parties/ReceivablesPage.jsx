...
function PaymentForm({ row, currency, onClose, bankAccounts }) {
  ...
  return (
    <div className="bg-navy/80 border border-accent/20 rounded-xl p-4 space-y-3 animate-fade-in">
      ...
      <p className="text-xs text-text-muted">
        ...
        {row._customerName && <> · <span className="text-accent">{row._customerName}</span></>}
      </p>
...
export default function ReceivablesPage() {
  ...
  const { data, isLoading, refetch } = useOutstandingBalances('receivable', { withAging: true })
  ...
  if (isMobile) {
    return (
      <MobileOutstanding
        ...
        partyCount={new Set(rows.map((r) => r._customerId).filter(Boolean)).size}
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'Invoice #',
      render: (r) => r._invoice
        ? <span className="font-mono text-xs text-accent">{r._invoice}</span>
