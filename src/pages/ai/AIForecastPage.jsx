    ...
    <div className={cn(
      ...
      <div className="flex items-end justify-between mt-0.5">
        ...
        {isPositive !== null && isPositive !== undefined && (
          <span className={cn(
            'flex items-center gap-0.5 text-small font-bold px-1.5 py-0.5 rounded-full',
      ...
      {sub && <span className="text-small text-text-muted leading-tight mt-0.5">{sub}</span>}
    </div>
  )
}
...
function ConfBadge({ label, score }) {
  const color = label === 'High'   ? 'text-positive bg-positive/10 border-positive/30'
    : label === 'Medium' ? 'text-highlight bg-highlight/10 border-highlight/30'
...
function ModelBadge({ dataSource, modelType }) {
  ...
  return (
    <span className={cn(
      ...
      strong ? 'text-accent bg-accent/10 border-accent/30' : 'text-text-muted bg-glass border-glass'
    )}>
...
function AnomalyRiskChip({ score, count }) {
  ...
  const color = pct >= 60 ? 'text-negative bg-negative/10 border-negative/30'
    : pct >= 30 ? 'text-highlight bg-highlight/10 border-highlight/30'
...
function InsightItem({ insight }) {
  ...
  return (
    <div className="flex gap-3 py-2.5 border-b border-glass/40 last:border-0">
      <div className={cn('mt-0.5 shrink-0', isWarn ? 'text-highlight' : isRec ? 'text-positive' : 'text-accent')}>
...
function DataSufficiencyBanner({ ds }) {
  ...
  const cfg = {
    insufficient: {
      icon: AlertTriangle,
      color: 'border-highlight/40 bg-highlight/5 text-highlight',
...
const RISK_LEVEL_STYLE = {
  ...
  warning:  'border-highlight/40 bg-highlight/5 text-highlight',
  ...
  return (
    <div className={cn('flex gap-3 p-3 rounded-xl border', style)}>
      ...
      <div>
        ...
        <p className="text-small opacity-80 leading-relaxed">{indicator.message}</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="group">
      <div className="flex items-center justify-between mb-1">
        ...
        <span className="text-small font-bold text-text-primary">{pct}%</span>
      </div>
  ...
  return (
    <div className="absolute inset-0 z-10 bg-navy/70 backdrop-blur-sm flex flex-col items-center justify-center rounded-xl gap-4">
      <div className="relative flex h-16 w-16 items-center justify-center">
        <div className="absolute inset-0 rounded-full border-4 border-glass border-t-accent animate-spin" />
        <BrainCircuit className="h-7 w-7 text-accent animate-pulse" />
      </div>
...
function CatTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-charcoal border border-glass p-2.5 rounded-lg shadow-lg text-xs">
      ...
      <p className="text-accent font-bold">{formatCurrency(v, currency)}</p>
        ...
        <div className="premium-card p-5">
          ...
          {momentum && (
            <div className="mt-3 pt-3 border-t border-glass/40 flex items-center gap-2 flex-wrap">
              <span className="text-small text-text-muted">Momentum:</span>
  ...
  return (
    <div className="premium-card p-5">
      <h3 className="flex items-center gap-2 text-sm font-bold text-text-primary mb-4 border-b border-glass pb-2">
        ...
        Category Breakdown
        <span className="text-xs font-normal text-text-muted ml-1">(last 3 months)</span>
      </h3>
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="premium-card p-5">
            <h2 className="flex items-center gap-2 text-sm font-bold text-text-primary mb-4 border-b border-glass pb-2">
              <Settings2 className="h-4 w-4 text-accent" /> Parameters
            </h2>
            <div className="space-y-3">
          ...
          {result?.modelMeta && !result?.dataSufficiency?.isInsufficient && (
            <div className="premium-card p-4 space-y-1.5 text-xs text-text-secondary">
              <h3 className="text-xs font-semibold text-text-primary border-b border-glass pb-1.5 mb-2 flex items-center gap-1.5">
                <CircleDot className="h-3 w-3 text-accent" /> Model Info
              </h3>
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="premium-card p-6">
            <div className="flex items-center justify-between border-b border-glass pb-3 mb-5">
              ...
              {mutation.isPending && (
                <span className="text-xs text-accent font-medium animate-pulse flex items-center gap-1.5">
            ...
            <div className="relative" style={{ minHeight: 380 }}>
              ...
              {result?.dataSufficiency?.isInsufficient ? (
                <div className="h-[380px] flex flex-col items-center justify-center gap-4 text-center px-8">
                  <AlertTriangle className="h-10 w-10 text-highlight/60" />
                  <div>
                ...
                <div className="h-[380px] flex items-center justify-center text-text-muted text-sm">
                  Select parameters and click <strong className="mx-1 text-accent">Run AI Model</strong>.
                </div>
              ) : null}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
        <div className="lg:col-span-1 space-y-4">
          ...
          {result?.insights?.length > 0 && (
            <div className="premium-card p-5 border-accent/20 bg-accent/5">
              <h2 className="flex items-center gap-2 text-sm font-bold text-text-primary mb-3 border-b border-glass pb-2">
                <Zap className="h-4 w-4 text-accent" /> Insights
              </h2>
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="premium-card p-6">
            <div className="flex items-center justify-between border-b border-glass pb-3 mb-5">
              ...
              {mutation.isPending && (
                <span className="text-xs text-accent animate-pulse flex items-center gap-1.5">
            ...
            <div className="relative" style={{ minHeight: 380 }}>
              ...
              {result?.dataSufficiency?.isInsufficient ? (
                ...
                <div className="h-[380px] flex items-center justify-center text-text-muted text-sm">
                  Click <strong className="mx-1 text-accent">Generate Forecast</strong> to begin.
                </div>
              ) : null}
            </div>
          </div>
...
function ScenarioTab() {
  ...
  const PRESETS = [
    ...
    { label: 'Cost spike +25%',   rev: 1.00, exp: 1.25, color: 'text-highlight'  },
  ...
  return (
    <div className="space-y-5">
      {/* Header info */}
      <div className="premium-card p-5 border-accent/20 bg-accent/5 flex gap-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="premium-card p-5">
            <h2 className="flex items-center gap-2 text-sm font-bold text-text-primary mb-4 border-b border-glass pb-2">
              <Settings2 className="h-4 w-4 text-accent" /> Scenario Setup
            </h2>
            <div className="space-y-4">
              <Select label="Target Metric" options={METRICS} value={metric} onChange={setMetric} />
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full text-xs bg-glass border border-glass rounded-lg px-3 py-2 text-text-primary placeholder-text-muted focus:outline-none focus:border-accent/50 focus:ring-1 focus:ring-accent/20"
                />
              </div>
        ...
        <div className="lg:col-span-3 space-y-4">
            ...
            <div className="relative" style={{ minHeight: 380 }}>
              ...
              {result ? (
                ...
                <div className="h-[380px] flex flex-col items-center justify-center text-text-muted gap-3">
                  ...
                  <p className="text-sm">Adjust parameters and click <strong className="text-accent">Run Simulation</strong></p>
          ...
          {result?.riskIndicators?.length > 0 && (
            <div className="premium-card p-5">
              <h3 className="flex items-center gap-2 text-sm font-bold text-text-primary mb-3 border-b border-glass pb-2">
                <AlertTriangle className="h-4 w-4 text-highlight" /> Scenario Risk Analysis
              </h3>
          ...
          {result?.insights?.length > 0 && (
            <div className="premium-card p-5 border-accent/20 bg-accent/5">
              <h3 className="flex items-center gap-2 text-sm font-bold text-text-primary mb-3 border-b border-glass pb-2">
                <Zap className="h-4 w-4 text-accent" /> Scenario Insights
              </h3>
          ...
          {healthData && (
            <div className="flex items-center gap-2 text-xs bg-glass border border-glass rounded-lg px-3 py-2">
              <span className={cn(
                ...
                healthData.lstmReady ? 'bg-accent animate-pulse' : 'bg-positive animate-pulse'
              )} />
