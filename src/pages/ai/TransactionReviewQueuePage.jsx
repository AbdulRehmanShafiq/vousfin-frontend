...
function ReviewList() {
  ...
  if (isLoading) return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {[...Array(6)].map((_, i) => <div key={i} className="h-52 bg-gray-100 animate-pulse rounded-xl" />)}
    </div>
  )
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs text-gray-500">{items.length} awaiting review on this page</p>
        <button
          onClick={confirmAll}
          disabled={bulkBusy || readyCount === 0}
          className="flex items-center gap-1.5 text-xs font-medium bg-green-600 hover:bg-green-700 disabled:opacity-40 text-white px-3 py-1.5 rounded-lg"
        >
  ...
  return (
    <div className="overflow-x-auto rounded-xl border border-gray-200">
      <table className="w-full text-sm">
        ...
        <tbody>
          {items.map(d => (
            <tr key={d.draft_id} className="border-b border-gray-100 last:border-0">
              ...
              <td className="px-3 py-2 text-right">
                <button
                  ...
                  className="text-xs font-medium text-red-600 hover:text-red-700 disabled:opacity-40 border border-red-200 hover:border-red-300 px-2.5 py-1 rounded-lg"
                >
...
export default function TransactionReviewQueuePage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
      <div>
        <h1 className="text-xl font-bold text-gray-900">AI Review Queue</h1>
