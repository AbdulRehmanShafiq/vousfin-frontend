...
const RESULT_BADGE = {
  clear:          'bg-positive/15 text-positive border border-positive/20',
  flagged:        'bg-red-500/15 text-red-400 border border-red-500/20',
  pending_review: 'bg-highlight/15 text-highlight border border-highlight/20',
}
...
function RiskBar({ score }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-label text-text-muted">{pct}</span>
    </div>
  )
}
...
export default function AmlPage() {
  ...
  return (
    <div className="animate-fade-in pb-10 space-y-5 max-w-4xl">
      ...
      <div className="premium-card p-3.5 flex items-start gap-2 text-small text-text-secondary border-l-2 border-highlight/40">
        <ShieldAlert className="h-4 w-4 text-highlight mt-0.5 shrink-0" />
      ...
      <div className="flex gap-3 items-center">
        ...
        <span className="text-xs text-text-muted">{screenings.length} record{screenings.length !== 1 ? 's' : ''}</span>
      </div>
      ...
      {justifyTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="premium-card p-6 w-full max-w-md space-y-4">
            ...
            <p className="text-label text-text-muted text-right">{justifyText.length}/1000</p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setJustifyTarget(null)} className="px-3 py-1.5 rounded-lg text-small text-text-muted border border-glass hover:bg-glass-hover">Cancel</button>
      ...
      {strModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="premium-card p-6 w-full max-w-lg space-y-4">
            ...
            <div className="p-3 rounded-lg bg-glass-panel/40 border border-glass text-small text-text-secondary leading-relaxed whitespace-pre-wrap">
              {strModal.draftText}
            </div>
            <p className="text-label text-text-muted">This is a draft only. No report has been submitted. Review with your compliance officer before filing.</p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setStrModal(null)} className="btn-gradient px-4 py-1.5 rounded-lg text-small font-semibold">Close</button>
            </div>
          </div>
        </div>
      )}
            ...
            <div key={s._id} className="premium-card p-3.5 flex items-start gap-3">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-small font-semibold text-text-primary">{s.counterpartyName}</span>
                  <span className={`text-label px-2 py-0.5 rounded-full capitalize font-medium ${RESULT_BADGE[s.result] || ''}`}>
                    {(s.result || '').replace('_', ' ')}
                  </span>
                  <span className="text-label text-text-muted capitalize">{s.counterpartyType}</span>
                </div>
                <div className="flex items-center gap-3 mt-1">
                  <RiskBar score={s.riskScore} />
                  {s.flags?.length > 0 && (
                    <span className="text-label text-text-muted">{s.flags.join(' · ')}</span>
                  )}
                </div>
                {s.justification && (
                  <p className="text-label text-text-muted mt-1">Justification: {s.justification}</p>
                )}
              </div>
              {s.result === 'flagged' && (
                <div className="flex flex-col gap-1.5 shrink-0">
                  <button onClick={() => { setJustifyTarget(s); setJustifyText(s.justification || '') }}
                    className="text-label px-2.5 py-1 rounded-lg bg-highlight/10 text-highlight hover:bg-highlight/20 border border-highlight/20 whitespace-nowrap">
                    Justify
                  </button>
                  <button onClick={() => loadSTR.mutate(s._id)} disabled={loadSTR.isPending}
                    className="inline-flex items-center gap-1 text-label px-2.5 py-1 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20 whitespace-nowrap disabled:opacity-50">
