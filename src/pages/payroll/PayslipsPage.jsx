  ...
  return (
    <div className="animate-fade-in pb-10 space-y-5 max-w-3xl">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-xl bg-accent/15"><FileText className="h-5 w-5 text-accent" /></div>
        <div>
      ...
      <div className="premium-card p-4 space-y-3">
        <label className="flex flex-col gap-1 max-w-xs">
          <span className="text-label text-text-muted">Payroll run</span>
        ...
        {runId && (
          <button onClick={() => downloadBankFile('bank')} disabled={busy === 'bank'}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-small text-text-secondary border border-glass hover:bg-glass-hover disabled:opacity-50">
      ...
      <div className="premium-card p-4 space-y-3">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <p className="text-small font-semibold text-text-secondary">Each employee</p>
          <label className="flex items-center gap-2">
            <span className="text-label text-text-muted">Tax year</span>
        ...
        {employees.map(emp => (
          <div key={emp._id} className="flex items-center justify-between gap-3 border-b border-glass/50 pb-2">
            <span className="text-small text-text-primary">{emp.fullName} <span className="text-text-muted">({emp.code})</span></span>
            <div className="flex items-center gap-2">
              <button onClick={() => downloadPayslip(emp._id)} disabled={!runId || busy === `slip-${emp._id}`}
                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-label text-text-secondary border border-glass hover:bg-glass-hover disabled:opacity-40">
        ...
        {employees.length === 0 && <p className="text-small text-text-secondary">No employees yet.</p>}
      </div>
    </div>
  )
}
