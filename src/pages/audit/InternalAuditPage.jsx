  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)}
        className="btn-gradient inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-small font-semibold">
    ...
    <form onSubmit={(e) => { e.preventDefault(); mut.mutate() }}
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="sm:col-span-2">
          <label className="block text-label text-text-muted mb-1">Plan name *</label>
        ...
        <div className="sm:col-span-2">
          <label className="block text-label text-text-muted mb-1">Scope (what are you reviewing?)</label>
          <input value={form.scope} onChange={e => set('scope', e.target.value)}
            className="w-full px-3 py-2 rounded-lg border border-glass bg-glass-panel/40 text-small text-text-primary focus:outline-none focus:border-accent/40" />
        </div>
        <div>
          <label className="block text-label text-text-muted mb-1">Period start *</label>
          <input type="date" value={form.periodStart} onChange={e => set('periodStart', e.target.value)} required
            className="w-full px-3 py-2 rounded-lg border border-glass bg-glass-panel/40 text-small text-text-primary focus:outline-none focus:border-accent/40" />
        </div>
        <div>
          <label className="block text-label text-text-muted mb-1">Period end *</label>
          <input type="date" value={form.periodEnd} onChange={e => set('periodEnd', e.target.value)} required
            min={form.periodStart || undefined}
            className="w-full px-3 py-2 rounded-lg border border-glass bg-glass-panel/40 text-small text-text-primary focus:outline-none focus:border-accent/40" />
        </div>
        <div>
          <label className="block text-label text-text-muted mb-1">Sample method</label>
          <select value={form.sampleStrategy} onChange={e => set('sampleStrategy', e.target.value)}
            className="w-full px-3 py-2 rounded-lg border border-glass bg-glass-panel/40 text-small text-text-primary focus:outline-none">
        ...
        <div>
          <label className="block text-label text-text-muted mb-1">Sample size (1–100)</label>
      ...
      <div className="flex gap-2">
        ...
        <button type="button" onClick={() => setOpen(false)}
          className="px-4 py-2 rounded-lg text-small text-text-muted hover:text-text-primary hover:bg-glass-hover">
          Cancel
        </button>
      </div>
    </form>
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); mut.mutate() }}
      ...
      <textarea value={obs} onChange={e => setObs(e.target.value)} required rows={2}
        ...
        className="w-full px-3 py-2 rounded-lg border border-glass bg-glass-panel/40 text-xs text-text-primary resize-none focus:outline-none focus:border-accent/40" />
      <div className="flex items-center gap-2">
        ...
        <button type="submit" disabled={mut.isPending || !obs.trim()}
          className="btn-gradient px-3 py-1.5 rounded-lg text-label font-semibold disabled:opacity-50">
          {mut.isPending ? <Loader2 className="h-3 w-3 animate-spin inline" /> : 'Raise'}
        </button>
        <button type="button" onClick={() => setOpen(false)}
          className="text-label text-text-muted hover:text-text-primary">Cancel</button>
      </div>
    </form>
  )
}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className={`text-label uppercase tracking-wider px-1.5 py-0.5 rounded-full font-semibold ${STATUS_BADGE[plan.status] || ''}`}>
              {plan.status?.replace('_', ' ')}
            </span>
          </div>
          {plan.scope && <p className="text-label text-text-muted mt-0.5">{plan.scope}</p>}
      ...
      {expanded && (
        <div className="space-y-3 border-t border-glass/40 pt-3">
          <button onClick={drawSample} disabled={sampling}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-accent/30 text-accent text-xs font-medium hover:bg-accent/10 disabled:opacity-50">
          ...
          {sample !== null && (
            <div className="space-y-1.5">
              <p className="text-label text-text-muted uppercase tracking-wider">{sample.length} transaction(s) sampled</p>
              {sample.length === 0 && (
                <p className="text-xs text-text-secondary">No transactions found in this period.</p>
              )}
              {sample.map((tx) => (
                <div key={tx._id} className="flex items-start justify-between gap-3 p-2.5 rounded-lg bg-glass-panel/30 border border-glass/30">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-text-primary font-medium">{fmtAmt(tx.amount)}</span>
                      <span className="text-label text-text-muted capitalize">{tx.transactionType}</span>
                      <span className="text-label text-text-muted">{fmt(tx.transactionDate)}</span>
                    </div>
                    {tx.description && <p className="text-label text-text-muted mt-0.5 truncate">{tx.description}</p>}
          ...
          <div className="pt-1">
            <p className="text-label text-text-muted mb-1">Raise a finding without a specific transaction:</p>
...
function FindingsSection({ refreshKey }) {
  ...
  return (
    <div className="space-y-2">
      {findings.length === 0 && (
        <p className="text-small text-text-secondary">No findings yet. Draw a sample from a plan to raise findings.</p>
      )}
      {findings.map(f => (
        <div key={f._id} className="premium-card p-3.5 space-y-2">
          <div className="flex items-start gap-3">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <span className={`text-label uppercase tracking-wider px-1.5 py-0.5 rounded-full font-semibold ${RISK_BADGE[f.riskRating] || ''}`}>
                  {f.riskRating}
                </span>
                <span className={`text-label uppercase tracking-wider px-1.5 py-0.5 rounded-full font-semibold ${FINDING_STATUS_BADGE[f.status] || ''}`}>
                  {f.status?.replace('_', ' ')}
                </span>
              </div>
              <p className="text-small text-text-primary mt-1">{f.observation}</p>
              {f.managementResponse && (
                <p className="text-label text-text-muted mt-1 italic">Response: {f.managementResponse}</p>
              )}
              {f.targetResolutionDate && (
                <p className="text-label text-text-muted">Target: {fmt(f.targetResolutionDate)}</p>
              )}
            </div>
            <button onClick={() => setShowResponse(showResponse === f._id ? null : f._id)}
              className="text-label text-accent hover:underline shrink-0">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {BUCKETS.map(b => (
          <div key={b} className="premium-card p-3 text-center">
            ...
            <p className="text-label text-text-muted mt-1">{BUCKET_LABELS[b]}</p>
          </div>
        ))}
      </div>
      <div className="premium-card p-4">
        <p className="text-label text-text-muted uppercase tracking-wider mb-3">By risk level</p>
        <div className="flex flex-wrap gap-4">
          {['critical','high','medium','low'].map(r => (
            <div key={r} className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-text-primary capitalize">{r}</span>
              <span className="text-xs text-text-muted">{data.byRisk[r] ?? 0}</span>
            </div>
          ))}
        </div>
        <p className="text-label text-text-muted mt-3">Total open: <span className="text-text-primary font-medium">{data.total}</span></p>
      </div>
    </div>
  )
}
    ...
    <div className="animate-fade-in pb-10 space-y-8 max-w-4xl">
      ...
      <section className="space-y-4">
        ...
        {plansLoading ? (
          ...
          <p className="text-small text-text-secondary">No audit plans yet. Create one to get started.</p>
        ) : (
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-label text-text-muted">(how long unresolved findings have been open)</span>
        </div>
        <AgingSection />
      </section>
    </div>
