  ...
  return (
    <div className="animate-fade-in pb-10 space-y-5 max-w-4xl">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-xl bg-cyan/15"><Boxes className="h-5 w-5 text-cyan" /></div>
        <div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); if (valid) create.mutate() }} className="premium-card p-4 space-y-3">
        <p className="text-[13px] font-semibold text-text-primary">Add an asset</p>
        <div className="grid sm:grid-cols-2 gap-3">
          <input value={form.name} onChange={set('name')} placeholder="Asset name (e.g. Delivery Van)" className="w-full px-3 py-2 rounded-lg border border-glass bg-glass-panel/40 text-[13px] text-text-primary focus:outline-none focus:border-cyan/40" />
          <input value={form.category} onChange={set('category')} placeholder="Category (optional)" className="w-full px-3 py-2 rounded-lg border border-glass bg-glass-panel/40 text-[13px] text-text-primary focus:outline-none focus:border-cyan/40" />
          <label className="text-[12px] text-text-muted">Acquisition date
            <input type="date" value={form.acquisitionDate} onChange={set('acquisitionDate')} className="w-full px-3 py-2 rounded-lg border border-glass bg-glass-panel/40 text-[13px] text-text-primary focus:outline-none focus:border-cyan/40 mt-1" />
          </label>
          <label className="text-[12px] text-text-muted">Cost
            <input type="number" min={0} step="0.01" value={form.acquisitionCost} onChange={set('acquisitionCost')} placeholder="0" className="w-full px-3 py-2 rounded-lg border border-glass bg-glass-panel/40 text-[13px] text-text-primary focus:outline-none focus:border-cyan/40 mt-1" />
          </label>
          <label className="text-[12px] text-text-muted">Salvage value (end-of-life worth)
            <input type="number" min={0} max={form.acquisitionCost || undefined} step="0.01" value={form.salvageValue} onChange={set('salvageValue')} className="w-full px-3 py-2 rounded-lg border border-glass bg-glass-panel/40 text-[13px] text-text-primary focus:outline-none focus:border-cyan/40 mt-1" />
          </label>
          <label className="text-[12px] text-text-muted">Useful life (years)
            <input type="number" min={1} max={100} step="1" value={form.usefulLifeYears} onChange={set('usefulLifeYears')} className="w-full px-3 py-2 rounded-lg border border-glass bg-glass-panel/40 text-[13px] text-text-primary focus:outline-none focus:border-cyan/40 mt-1" />
          </label>
          <label className="text-[12px] text-text-muted sm:col-span-2">Depreciation method
        ...
        <div className="flex justify-end">
          <button type="submit" disabled={!valid || create.isPending} className="btn-gradient inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-[12.5px] font-semibold disabled:opacity-50">
              ...
              <div key={a._id} className="premium-card p-4">
                <div className="flex items-center gap-3 flex-wrap">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-[14px] font-semibold text-text-primary">{a.name}</span>
                      <span className={`text-[10.5px] uppercase tracking-wider ${STATUS_STYLE[a.status] || 'text-text-muted'}`}>{a.status.replace('_', ' ')}</span>
                    </div>
                  ...
                  <button onClick={() => setOpenId(openId === a._id ? null : a._id)} className="text-[12px] text-text-muted hover:text-text-primary inline-flex items-center gap-1">
                    Schedule <ChevronDown className={`h-3.5 w-3.5 transition-transform ${openId === a._id ? 'rotate-180' : ''}`} />
                  </button>
                  {a.status === 'active' && (
                    <>
                      <button onClick={() => depreciate.mutate(a._id)} disabled={depreciate.isPending} className="text-[12px] inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-glass text-text-secondary hover:text-text-primary hover:bg-glass-hover disabled:opacity-50">
                        <TrendingDown className="h-3.5 w-3.5" /> Post depreciation
                      </button>
                      <button onClick={() => { setDisposeFor(disposeFor === a._id ? null : a._id); setDisposal({ disposalDate: '', proceeds: '' }) }} className="text-[12px] inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-negative hover:bg-glass-hover">
                ...
                {disposeFor === a._id && (
                  <form onSubmit={(e) => { e.preventDefault(); if (disposal.disposalDate) dispose.mutate({ id: a._id, data: { disposalDate: disposal.disposalDate, proceeds: Number(disposal.proceeds || 0) } }) }}
                    ...
                    <label className="text-[12px] text-text-muted">Disposal date
                      <input type="date" required value={disposal.disposalDate} onChange={(e) => setDisposal(d => ({ ...d, disposalDate: e.target.value }))} className="w-full px-3 py-2 rounded-lg border border-glass bg-glass-panel/40 text-[13px] text-text-primary focus:outline-none focus:border-cyan/40 mt-1" />
                    </label>
                    <label className="text-[12px] text-text-muted">Sale proceeds
                      <input type="number" min={0} step="0.01" value={disposal.proceeds} onChange={(e) => setDisposal(d => ({ ...d, proceeds: e.target.value }))} placeholder="0" className="w-full px-3 py-2 rounded-lg border border-glass bg-glass-panel/40 text-[13px] text-text-primary focus:outline-none focus:border-cyan/40 mt-1" />
                    </label>
                    <button type="submit" disabled={dispose.isPending} className="px-3 py-2 rounded-lg bg-negative/15 text-negative text-[12.5px] font-semibold disabled:opacity-50">
