...
function CustomTooltip({ active, payload, label, currency, metricName, showScenarios }) {
  ...
  return (
    <div className="bg-charcoal/95 border border-glass/60 p-3 rounded-xl shadow-2xl min-w-[180px] backdrop-blur-sm">
      <p className="text-text-muted text-small mb-2.5 font-semibold uppercase tracking-wide border-b border-glass/40 pb-1.5">
        {label}
      </p>
      {items.map((entry, i) => (
        <div key={i} className="flex items-center justify-between gap-6 mb-1">
          <span className="flex items-center gap-1.5 text-small" style={{ color: entry.color }}>
...
function CustomLegend({ color, showScenarios }) {
  return (
    <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5 text-small text-text-secondary mt-2 justify-center">
...
export default function ForecastChart({
  ...
  return (
    <div>
      ...
      {hasScenarioData && (
        <div className="flex justify-end mb-2">
          <button
            ...
            className={`text-small font-medium px-3 py-1 rounded-full border transition-all ${
              showScenarios
      ...
      {/* Risk zone warning */}
      {showRiskZone && (
        <p className="text-xs text-negative/80 text-center mt-1.5 flex items-center justify-center gap-1.5">
