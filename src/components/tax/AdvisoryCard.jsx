  ...
  return (
    <div className="premium-card p-5 flex flex-col gap-3">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="shrink-0 rounded-full bg-positive/12 text-positive text-small font-bold px-2.5 py-1">
          Save ~<span className="num">{compactMoney(advisory.estimatedSavingPKR, currency)}</span>
        </span>
      </div>

      <p className="text-small text-text-secondary leading-relaxed">{advisory.explanation}</p>
      ...
      {review && advisory.riskWarning && (
        <div className="flex items-start gap-2 rounded-lg border border-negative/30 bg-negative-muted px-3 py-2">
          ...
          <p className="text-xs text-negative leading-snug">{advisory.riskWarning}</p>
        </div>
      )}
      ...
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="inline-flex items-center gap-1.5 self-start text-small font-semibold text-text-muted hover:text-accent transition-colors"
        aria-expanded={open}
      >
      ...
      {open && (
        <p className="text-small text-text-secondary border-l-2 border-glass pl-3">{advisory.legalRef}</p>
      )}
    </div>
  )
}
