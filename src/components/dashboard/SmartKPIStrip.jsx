...
function TrendBadge({ trend, label }) {
  ...
  return (
    <span className={cn(
      'inline-flex items-center gap-0.5 text-xs font-bold flex-shrink-0',
  ...
  const inner = (
    <div className={cn(
      ...
      <div className="flex-1 min-w-0">
        ...
        {subtitle && !loading && <p className="text-small text-text-muted leading-tight truncate">{subtitle}</p>}
      </div>
...
const SmartKPIStrip = memo(function SmartKPIStrip({ kpis = {}, revenueVsExpenses = [], loading, currency }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="md:hidden -mx-4 px-4">
        ...
        <div className="flex justify-center gap-1.5 mt-2">
          {primaryCards.map((c, i) => (
            <div key={c.key} className={cn('h-1 rounded-full transition-all', i === 0 ? 'w-4 bg-accent' : 'w-1.5 bg-glass-panel')} />
          ))}
        </div>
      </div>

      {/* Tablet: 2-up (roomier) · Desktop: 4-up */}
      <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-4 gap-3 stagger-rise">
        {primaryCards.map(({ key, ...card }) => (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 stagger-rise">
