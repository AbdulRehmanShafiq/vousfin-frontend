    ...
    <div className="flex flex-col items-center gap-1.5 group">
      ...
      <div className="relative h-12 w-12">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="text-small font-black leading-none" style={{ color }}>{score}</span>
        </div>
      </div>
      {/* label */}
      <div className="flex flex-col items-center gap-0.5">
        <Icon className="h-2.5 w-2.5" style={{ color }} />
        <span className="text-small font-semibold text-text-muted uppercase tracking-wide whitespace-nowrap">{label}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col gap-2 min-w-0">
      ...
      <div className="flex items-center justify-between gap-2">
        <p className="text-xs font-bold uppercase tracking-widest text-text-muted">Cash Flow Risk</p>
        <div className="flex items-center gap-1">
          <cfg.Icon className="h-3 w-3" style={{ color: cfg.color }} />
          <span className="text-small font-bold" style={{ color: cfg.color }}>{cfg.label}</span>
        </div>
      </div>
      {/* bar */}
      ...
      {/* labels */}
      <div className="flex justify-between text-small text-text-muted font-medium">
        <span>Critical</span>
      ...
      <div className="flex items-center justify-between gap-2 mb-4">
        ...
        {!isLoading && !insufficient && (
          <div className="flex items-center gap-1.5 flex-shrink-0">
            {conf && (
              <span
                className="px-2 py-0.5 rounded-full text-small font-bold uppercase tracking-wide"
            ...
            {view.estimated && (
              <span className="px-2 py-0.5 rounded-full text-small font-bold uppercase tracking-wide bg-glass-panel text-text-muted">
                Estimated
              </span>
            )}
          </div>
        ...
        <>
            ...
            <div className="flex items-center justify-between gap-x-6 gap-y-4 flex-wrap pt-4 border-t border-glass">
              ...
              <div className="flex items-center gap-2.5 flex-shrink-0">
                ...
                <div className="flex flex-col">
                  <p className="text-xs text-text-muted font-semibold leading-tight">/100</p>
                  <p className="text-xs font-bold uppercase tracking-wide leading-tight" style={{ color: overallColor }}>
          ...
          {view.drivers.length > 0 && (
            <div className="mt-4 pt-3 border-t border-glass">
              <p className="text-xs font-bold uppercase tracking-widest text-text-muted mb-1.5">What's driving this</p>
              <ul className="space-y-1">
                {view.drivers.map((d, i) => (
                  <li key={i} className="flex items-start gap-1.5 text-small text-text-secondary">
          ...
          {!view.estimated && trendData.length >= 2 && (
            <div className="mt-3 pt-3 border-t border-glass flex items-center justify-between gap-3">
              <div className="flex items-center gap-2.5">
                <span className="text-xs font-bold uppercase tracking-widest text-text-muted">Trend</span>
